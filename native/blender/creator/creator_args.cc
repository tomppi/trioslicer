...
static const char *parse_int_range_sep_search(const char *str, const char *str_end_test)
{
  ...
  if (str_end_test) {
    str_end_range = (const char *)memchr(str, '.', (str_end_test - str) - 1);
                                              ...
                                              const char **r_err_msg))[2]
{
  ...
  int(*values)[2] = (int (*)[2])MEM_mallocN(sizeof(*values) * len, __func__);
  int i = 0;
  while (true) {
    const char *str_end_range;
    const char *str_end = strchr(str, sep);
                                  ...
                                  const char *script_id)
{
  ...
  if (c_py->has_win) {
    ...
    CTX_wm_window_set(C, (struct wmWindow *)c_py->wm->windows.first);
  }
  else {
    c_py->win = NULL;
    fprintf(stderr,
...
static int arg_handle_print_version(int (argc),
                                    const char **(argv),
                                    void *(data))
{
  print_version_full();
...
static const char arg_handle_arguments_end_doc[] =
    ...
    "'sys.argv'.";
static int arg_handle_arguments_end(int (argc),
                                    const char **(argv),
                                    void *(data))
{
  return -1;
...
static int arg_handle_crash_handler_disable(int (argc),
                                            const char **(argv),
                                            void *(data))
{
...
static int arg_handle_abort_handler_disable(int (argc),
                                            const char **(argv),
                                            void *(data))
{
...
static int arg_handle_debug_exit_on_error(int (argc),
                                          const char **(argv),
                                          void *(data))
{
...
static int arg_handle_background_mode_set(int (argc),
                                          const char **(argv),
                                          void *(data))
{
  print_version_short();
...
static int arg_handle_log_show_basename_set(int (argc),
                                            const char **(argv),
                                            void *(data))
{
...
static int arg_handle_log_show_backtrace_set(int (argc),
                                             const char **(argv),
                                             void *(data))
{
...
static int arg_handle_log_show_timestamp_set(int (argc),
                                             const char **(argv),
                                             void *(data))
{
...
#  endif
  ...
  BLI_args_print((struct bArgs *)data);
  return 0;
}
...
static int arg_handle_debug_mode_generic_set(int (argc),
                                             const char **(argv),
                                             void *data)
{
...
static int arg_handle_debug_mode_io(int (argc),
                                    const char **(argv),
                                    void *(data))
{
  G.debug |= G_DEBUG_IO;
...
static int arg_handle_debug_mode_all(int (argc),
                                     const char **(argv),
                                     void *(data))
{
  G.debug |= G_DEBUG_ALL;
...
static int arg_handle_debug_mode_libmv(int (argc),
                                       const char **(argv),
                                       void *(data))
{
  libmv_startDebugLogging();
...
static int arg_handle_debug_mode_cycles(int (argc),
                                        const char **(argv),
                                        void *(data))
{
  CCL_start_debug_logging();
...
static int arg_handle_debug_mode_memory_set(int (argc),
                                            const char **(argv),
                                            void *(data))
{
  MEM_set_memory_debug();
...
static int arg_handle_debug_gpu_set(int (argc),
                                    const char **(argv),
                                    void *(data))
{
...
static int arg_handle_debug_fpe_set(int (argc),
                                    const char **(argv),
                                    void *(data))
{
...
static int arg_handle_factory_startup_set(int (argc),
                                          const char **(argv),
                                          void *(data))
{
  G.factory_startup = 1;
...
static int arg_handle_enable_event_simulate(int (argc),
                                            const char **(argv),
                                            void *(data))
{
...
static int arg_handle_native_pixels_set(int (argc),
                                        const char **(argv),
                                        void *(data))
{
...
static int arg_handle_without_borders(int (argc),
                                      const char **(argv),
                                      void *(data))
{
...
static int arg_handle_window_maximized(int (argc),
                                       const char **(argv),
                                       void *(data))
{
...
static int arg_handle_no_window_focus(int (argc),
                                      const char **(argv),
                                      void *(data))
{
...
static int arg_handle_start_with_console(int (argc),
                                         const char **(argv),
                                         void *(data))
{
...
static int arg_handle_audio_disable(int (argc),
                                    const char **(argv),
                                    void *(data))
{
...
static int arg_handle_engine_set(int argc, const char **argv, void *data)
{
  ...
  if (argc >= 2) {
    if (STREQ(argv[1], "help")) {
      ...
      for (type = (RenderEngineType *)R_engines.first; type; type = type->next) {
        printf("\t%s\n", type->idname);
      }
      exit(0);
    }
...
static int arg_handle_load_file(int (argc), const char **argv, void *data)
{
  bContext *C = (bContext *)data;
  ReportList reports;
  bool success;
...
static int arg_handle_load_last_file(int (argc), const char **(argv), void *data)
{
  ...
  const RecentFile *recent_file = (const RecentFile *)G.recent_files.first;
  const char *fake_argv[] = {recent_file->filepath};
  return arg_handle_load_file((int)(ARRAY_SIZE(fake_argv)), fake_argv, data);
}
