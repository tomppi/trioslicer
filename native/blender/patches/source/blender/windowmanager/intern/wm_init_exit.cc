...
void WM_init_opengl(void)
{
  ...
  /* Nothing calls GPU_backend_type_selection_detect() on this path, so the
   * selection is still GPU_BACKEND_NONE here and GPU_backend_supported()
   * reports "no backend" -- which silently skipped context creation entirely
   * and left the draw manager to dereference a null context.  Offscreen
   * rendering in background mode is OpenGL only, so select it explicitly. */
  if (G.background && GPU_backend_type_selection_get() == GPU_BACKEND_NONE) {
    GPU_backend_type_selection_set(GPU_BACKEND_OPENGL);
    fprintf(stderr, "[wm] background: forced GPU_BACKEND_OPENGL\n");
  }

  if (!GPU_backend_supported()) {
    fprintf(stderr, "[wm] WM_init_opengl: GPU_backend_supported() == false, bailing\n");
    return;
  }

  fprintf(stderr, "[wm] WM_init_opengl: creating offscreen GL context\n");
