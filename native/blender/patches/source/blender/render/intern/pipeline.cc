...
{
  fprintf(stderr, "[re] RE_gpu_context_get: gpu_context=%p gl_context=%p\n",
          re->gpu_context,
          re->gl_context);
  if (re->gpu_context == nullptr) {
    /* Background mode reaches rendering without the draw manager ever having
     * created a context (WM_init_opengl is only reached from the DRW context
     * enable path, which a `blender -b` render never hits).  Calling
     * GPU_context_create with a null context initialises the GPU backend with
     * no current GL context, and the backend's glGetString() then dereferences
     * null and kills the process.  Create the offscreen context here, the same
     * way RE_engine_gpu_context_create does for the viewport path. */
    if (re->gl_context == nullptr) {
      re->gl_context = WM_opengl_context_create();
      fprintf(stderr, "[re] created offscreen gl_context=%p\n", re->gl_context);
      if (re->gl_context) {
        WM_opengl_context_activate(re->gl_context);
      }
    }
    re->gpu_context = GPU_context_create(nullptr, re->gl_context);
    fprintf(stderr, "[re] gpu_context=%p\n", re->gpu_context);
  }
