...
GHOST_TSuccess GHOST_ContextEGL::initializeDrawingContext()
{
  ...
  fprintf(stderr,
          "[egl] init: nativeWindow=%p nativeDisplay=%p api=%d major=%d minor=%d\n",
          (void *)m_nativeWindow,
  ...
  if (!EGL_CHK(::eglMakeCurrent(m_display, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT))) {
    fprintf(stderr, "[egl] eglMakeCurrent(bootstrap) FAILED\n");
    goto error;
  }
  fprintf(stderr, "[egl] initialized %d.%d\n", egl_major, egl_minor);
  if (!bindAPI(m_api)) {
    fprintf(stderr, "[egl] bindAPI FAILED\n");
    goto error;
  }
  fprintf(stderr, "[egl] bindAPI ok\n");
  ...
  if (m_api == EGL_OPENGL_ES_API && epoxy_egl_version(m_display) >= 12) {
    ...
    /* The spec requires EGL_RENDERABLE_TYPE here.  Without it eglChooseConfig
     * falls back to EGL_OPENGL_ES_BIT and can hand back a config that cannot
     * create the ES3 context requested further down. */
    attrib_list.push_back(EGL_RENDERABLE_TYPE);
    attrib_list.push_back(EGL_OPENGL_ES3_BIT_KHR);

    if (m_contextMajorVersion == 1 || m_contextMajorVersion == 2 ||
        m_contextMajorVersion == 3)
    {
      /* Surface type is selected once, below, from m_nativeWindow. */
    }
    else {
      fprintf(stderr,
  ...
  fprintf(stderr, "[egl] chooseConfig num_config=%d\n", (int)num_config);
...
//  }
  ...
  fprintf(stderr, "[egl] makeCurrent ok -- context is live\n");
